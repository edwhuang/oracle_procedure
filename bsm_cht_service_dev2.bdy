create or replace package body iptv.BSM_CHT_SERVICE_DEV2 is
  ...
  BEGIN
    for i in c1 loop
      ...
      if v_cat = 'LITV' then
        P_GATEWAY  := link_set.link_set.p_hinet_url ||
                      '/BSM_HINET_SERVICE/bsm_hinet.ashx';
        P_GATEWAY2 := link_set.link_set.p_hinet_url_2 ||
                      '/BSM_HINET_SERVICE/bsm_hinet.ashx';
      ...
      exception
        when others then
          req := utl_http.begin_request(P_GATEWAY2, 'POST', 'HTTP/1.1');
          utl_http.set_header(req, 'Content-Length', length(content));
          utl_http.write_text(req, content);
          resp := utl_http.get_response(req);
          dbms_lob.createtemporary(pagelob, true);
    ...
    for i in c1 loop
      ...
      exception
        when no_data_found then
          begin
            P_ALIAS     := nvl(i.cht_alias, i.serial_id);
            ...
            begin
              ...
              jsonobj := json(pagelob);
              if json_ext.get_string(jsonobj, 'error.name') =
                 'JSONRPCError' then
                v_result := 'ERROR';
              end if;
              dbms_lob.freetemporary(pagelob);
            exception
              when others then
                req := utl_http.begin_request(P_GATEWAY2,
                                              'POST',
                                              'HTTP/1.1');
                utl_http.set_header(req, 'Content-Length', length(content));
                utl_http.write_text(req, content);
  ...
  function ATMMultiplebills(p_purchase_pk_no number, p_otpw varchar2)
    ...
    cursor c1 is
      select a.amount, a.purchase_date
        from bsm_purchase_mas a, bsm_purchase_item b, bsm_package_mas c
       where b.mas_pk_no = a.pk_no
         and c.package_id = b.package_id
         and a.pk_no = p_purchase_pk_no;
  ...
  BEGIN
    for i in c1 loop
      ...
      content := replace(content,
                         'P_Multiplebills',
                         '1^' || to_char(i.purchase_date, 'YYYYMMDD') ||
                         to_char(i.purchase_date, 'YYYYMMDD') ||
                         lpad(to_char(i.amount), 6, '0'));
