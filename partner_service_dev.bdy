CREATE OR REPLACE PACKAGE BODY IPTV.PARTNER_SERVICE_DEV is
    ...
    if v_action = 'create' then
      ...
      if v_client_id is null then
        ...
        begin
          if v_vendor is not null then
            begin
              select pay_type, nvl(v_p_recurrent,default_recurrent_type), nvl(ext_type, 'E')
       ...
       /*   if (p_start_form_active is not null and
            ...
            begin
             ...
             /* if (upper(v_vendor) not in ('CNS')) then
              ...
              update bsm_client_details a
               ...
               where a.src_pk_no = v_purchase_pk_no
                 ...
                 set x.client_id       = v_client_Id,
                     ...
                     x.p_service_end   = nvl(v_cal_expire_date,trunc(add_months(v_action_date,
                                                       v_months) + v_d_day) -
                                      (1 / (24 * 60 * 60))),
              ...
              v_result := '{"result_code":"BSM-00000","result_message":"","purchase_no":"' ||
                          v_Purchase_No || '","client_id":"' || v_client_id ||
                          '","service_start_date":"' ||  nvl(to_char(v_action_date,'YYYY/MM/DD HH24:MI:SS'), to_char(v_action_date,'YYYY/MM/DD HH24:MI:SS'))||
                          '","service_end_date":"' || nvl(to_char(v_cal_expire_date,'YYYY/MM/DD HH24:MI:SS'),v_end_date) ||'"}';
              ...
              if v_expire_date < sysdate then
                select max(end_date)
                  ...
                  from (select trunc(add_months(v_expire_date,
                               ...
                               (1 / (24 * 60 * 60)) end_date
                          from dual
                        connect by trunc(add_months(to_date('2021/04/20',
                                                            'YYYY/MM/DD'),
                                                    rownum * v_months) +
                                         v_d_day * rownum) -
                                   (1 / (24 * 60 * 60)) <
      ...
      declare
        cursor c1 is
          select a.mas_no, pk_no
            from bsm_purchase_mas a
           where replace(replace(a.src_no,'so-net','sonet'), 'GOOGLE', 'IAB') =
