CREATE OR REPLACE PACKAGE BODY IPTV."BSM_RECURRENT_UTIL" is
  ...
  begin
    ...
    if v_recurrent_type = 'HINET' then
      ...
      update bsm_client_details a
         set a.end_date = sysdate, a.extend_days = 0
          ...
          if i.card_expiry < to_char(sysdate, 'yyyymm') then
            ...
            begin
              ...
              exception
                when others then
                  update bsm_purchase_mas a
                     set a.response_code = null, a.status_flg = 'F'
            ...
            exception
              when no_data_found then
                dbms_output.put_line('break' || i.serial_id);
            end;
          end if;
    ...
    begin
      for i in c1 loop
        begin
          ...
          begin
            rw_result := bsm_ios_gateway.get_Receipt_Data(i.pk_no,
                                                          '',
                                                          '',
                                                          i.ios_product_code);
            ...
            begin
              v_flg             := to_CHAR(json_ext.get_number(jsonobj,
                                                               'result.status'));
              auto_renew_status := to_CHAR(json_ext.get_number(jsonobj,
                                                               'result.auto_renew_status'));
            exception
              when others then
                v_flg := null;
            end;
            ...
            begin
              pending_list := json_ext.get_json_list(jsonobj,
          ...
          if v_flg = '21006' or auto_renew_status = '0' then
            ...
            if pending_list is not null then
              for j in 1 .. pending_list.COUNT() loop
                ...
                begin
                  auto_renew_status_1     := json_ext.get_string(rep_data,
                                                                 'auto_renew_status');
                  original_transaction_id := json_ext.get_string(rep_data,
                                                                 'original_transaction_id');
                
                  if auto_renew_status_1 = '0' then
                    update bsm_recurrent_mas c
                       set c.status_flg = 'B', c.stop_date = sysdate
                     where c.recurrent_type = 'IOS'
                       and c.src_no in
                           (select mas_no
                              from bsm_purchase_mas d
                             where d.ios_org_trans_id =
                                   original_transaction_id);
                    commit;
                  end if;
    ...
    declare
      cursor c1 is
        Select b.serial_id,
               ...
               e.duration_by_month,
               e.duration_by_day,
               nvl(g.p_service_start, d.end_date) p_cal_star
          from bsm_recurrent_mas  a,
               bsm_purchase_mas   b,
               bsm_purchase_item  c,
               bsm_client_details d,
      ...
      for i in c1 loop
        ...
        begin
          select max(end_date)
            ...
            from (select trunc(add_months(i.p_cal_star, rownum * i.duration_by_month) +
                               i.duration_by_day * rownum) - (1 / (24 * 60 * 60)) end_date
                    from dual
                  connect by trunc(add_months(i.p_cal_star,
                                              rownum * i.duration_by_month) +
                                   i.duration_by_day * rownum) - (1 / (24 * 60 * 60)) <
                             trunc(add_months(sysdate,  i.duration_by_month) + 0 -
                                   (1 / (24 * 60 * 60))));
                           
        
          update bsm_client_details c
             set c.start_date = trunc(sysdate - 1),
                 c.end_date   = v_cal_expire_date
