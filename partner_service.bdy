CREATE OR REPLACE PACKAGE BODY IPTV.PARTNER_SERVICE is
  ...
  begin
    ...
    begin
      ...
      begin
        v_bsm_order_id := json_ext.get_string(j_order, 'bsm_order_id');
      exception
        when others then
          v_bsm_order_id := null;
      end;
      ...
      begin
        v_p_recurrent := json_ext.get_string(j_order, 'recurrent_type');
      exception
        when others then
          v_p_recurrent := null;
      end;
    ...
    if v_action = 'create' then
      begin
        select a.mas_no, a.pk_no
         ...
         where replace(a.src_no, 'GOOGLE', 'IAB') =
            ...
            when no_data_found then
              if v_client_id is not null then
                ...
                begin
                  ...
                  begin
                    v_result := bsm_client_service.Check_And_Register_Client(in_client_info => in_client_info);
                  exception
                    when others then
                      null;
                  end;
                ...
                exception
                  ...
                  when others then
                    ...
                    begin
                      ...
                      if instr(v_errm, 'ORA-00001') >= 0 then
                        null;
                      else
      ...
      if v_client_id is null then
        ...
        begin
          if v_vendor is not null then
            begin
              select pay_type,
                     nvl(v_p_recurrent, default_recurrent_type),
                     nvl(ext_type, 'E')
            ...
            exception
              when no_data_found then
                        insert into acg_vendor_payment_mas(pay_type,default_recurrent_type,ext_type,vendor)
                values(v_vendor,'R','E',upper(v_vendor));
                v_pay_type:= v_vendor;
                v_recurrent_type :='R';
                v_start_type := 'E';
                commit;
          ...
          (upper(v_vendor) = 'SONET') or (upper(v_vendor) = 'CNS') then */
          if v_action_date is not null then
            ...
            begin
              select nvl(a.duration_by_month, 0),
                ...
                from bsm_package_mas a
               where a.package_id = v_package_id;
              /* if (upper(v_vendor) not in ('CNS')) then
                declare
              ...
              if v_expire_date is null then
                v_expire_date := v_action_date;
              end if;
              if v_expire_date < sysdate then
                select max(end_date)
                  into v_cal_expire_date
                  from (select trunc(add_months(v_expire_date,
                                                rownum * v_months) +
                                     v_d_day * rownum) -
                               (1 / (24 * 60 * 60)) end_date
                          from dual
                        connect by trunc(add_months(v_expire_date,
                                                    rownum * v_months) +
                                         v_d_day * rownum) -
                                   (1 / (24 * 60 * 60)) <
                                   trunc(add_months(sysdate, 1) + 0 -
                                         (1 / (24 * 60 * 60))));
              else
                v_cal_expire_date := v_expire_date;
              end if;
              update bsm_client_details a
                 set a.start_date = nvl(v_action_date, a.start_date),
                     a.end_date   = nvl(v_cal_expire_date, a.end_date)
              ...
              update parent_order x
                 set x.client_id       = v_client_Id,
                     ...
                     x.p_service_start = nvl(v_action_date, v_action_date),
                     x.p_service_end   = nvl(v_cal_expire_date,
              ...
              v_result := '{"result_code":"BSM-00000","result_message":"","purchase_no":"' ||
                          v_Purchase_No || '","client_id":"' || v_client_id ||
                          '","service_start_date":"' ||
                          nvl(to_char(v_action_date,
                                      'YYYY/MM/DD HH24:MI:SS'),
                              to_char(v_action_date, 'YYYY/MM/DD HH24:MI:SS')) ||
                          '","service_end_date":"' ||
                          nvl(to_char(v_cal_expire_date,
                                      'YYYY/MM/DD HH24:MI:SS'),
                              v_end_date) || '"}';
            ...
            begin
              ...
              update bsm_client_details a
                 set a.end_date = v_cal_expire_date
               where a.src_pk_no = v_purchase_pk_no
                 and a.package_id = v_package_id
                 and rownum <= 1;
              commit;
        ...
        cursor c1 is
          select a.mas_no, pk_no,a.mas_date
           ...
           where replace(replace(a.src_no, 'so-net', 'sonet'),
                         'GOOGLE',
                         'IAB') =
                 v_order_pre_fix || replace(v_order_id, 'so-net', 'sonet')
              or (a.mas_no = v_bsm_order_id);
      begin
        /*    begin
           
             select a.mas_no, pk_no into
              v_Purchase_No, v_Purchase_Pk_No
            from bsm_purchase_mas a
           where replace(replace(a.src_no, 'so-net', 'sonet'),
                         'GOOGLE',
                         'IAB') =
                 v_order_pre_fix || replace(v_order_id, 'so-net', 'sonet')
              or (a.mas_no = v_bsm_order_id) and rownum <=1;
        exception
          when no_data_found then
        ...
        for i in c1 loop
          ...
          update iptv.tstar_order c
             set c.status_flg = 'C'
           where c.orderid = v_order_id;
        ...
        when no_data_found then
          ...
          begin
            select a.src_order_id
              into v_order_no
              from parent_order a
             where replace(a.src_order_id, 'GOOGLE', 'IAB') =
                   v_order_pre_fix || v_order_id
               and rownum <= 1;
            update parent_order a
               set status_flg = 'C'
      ...
      if v_Purchase_No is null or v_Purchase_No = '' then
        v_result := '{"result_code":"BSM-00802","result_message":"no order found","purchase_no":"' ||
                    v_Purchase_No || '","client_id":"' || v_client_id || '"}';
      else
      ...
      begin
        for cprec in cp loop
        
          v_Purchase_No    := cprec.mas_no;
          v_Purchase_Pk_No := cprec.pk_no;
        
          update bsm_client_details a
             set a.start_date = nvl(v_action_date, sysdate),
                 a.end_date   = nvl(v_expire_date,
                                    add_months(nvl(v_action_date, sysdate), 1)),
                 a.status_flg = 'P'
           where a.src_pk_no = v_Purchase_Pk_No;
      ...
      begin
          ...
          select a.mas_no,
                 ...
                 case
                   when duration_by_MONTH > 0 then
                    trunc(add_MONTHS(d.start_date,
                                     trunc(months_between(sysdate,
                                                          d.start_date) /
                                           c.duration_by_MONTH) + 1)) + 1 -
                    (1 / (24 * 60 * 60))
                   else
                    trunc(d.start_date) +
                    (trunc((trunc(sysdate) - trunc(d.start_date)) /
                           duration_by_day) + 1) * duration_by_day + 1 -
                    (1 / 24 * 60 * 60)
                 end
           ...
           where b.mas_pk_no = a.pk_no
             ...
             and replace(a.src_no, 'GOOGLE', 'IAB') =
                 v_order_pre_fix || v_order_id;
        exception
          when no_data_found then
            if v_bsm_order_id is not null then
        ...
        update bsm_client_details a
           set a.status_flg = 'P', a.end_date = v_expire_date
         where a.src_pk_no = v_Purchase_Pk_No;
