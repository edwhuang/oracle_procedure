create or replace PACKAGE BODY BSM_PURCHASE_POST is
  ...
  begin
    ...
    begin
      v_payload := purchase_msg_type(v_mac_address,
                                     p_Pk_No,
                                     v_mas_no,
                                     'promo');
    ...
    --
    if v_pay_type in
       ('兌換券', '匯款', 'ATM', '其他', 'REMIT', '中華電信ATM') then
      declare
        cursor c1 is
    ...
    BSM_client_service.Set_subscription_r(null,
                                          v_mac_address,
                                          refresh_client);
    ...
    if v_pay_type in ('匯款', 'ATM', '其他', 'REMIT', '中華電信ATM') then
        ...
        v_payload            purchase_msg_type;
      begin
        v_payload := purchase_msg_type(v_mac_address, p_Pk_No,p_Pk_No, 'send_pur_sms');
        dbms_aq.enqueue(queue_name         => 'purchase_msgb_queue',
                        enqueue_options    => v_enqueue_options,
                        message_properties => v_message_properties,
                        payload            => v_payload,
                        msgid              => v_message_handle);
  ...
  /*    declare
      ...
      begin
        If v_owner_phone_no Is Not Null Then
          if send_bonuswinner_cups(v_owner_phone_no,
                                   v_mac_address,
                                   v_mas_no,
                                   v_Purchase_Amount,
                                   v_pay_type,
      ...
      cursor c1 is
        Select b.src_pk_no,
               ...
               trunc(bsm_recurrent_util.get_service_end_date(c.package_cat_id1,
                                                             b.serial_id) ) next_pay
          from bsm_recurrent_mas a, bsm_client_details b, bsm_package_mas c
         where a.recurrent_type in ('LiPay','LiPayN')
           and a.status_flg = 'P'
           and b.src_no = a.src_no
           and c.package_id = b.package_id
           and b.serial_id = v_client_id;
      v_msg varchar2(2048);
    begin
      for i in c1 loop
       ...
       declare
        ...
        v_payload            purchase_msg_type;
      begin
        v_payload := purchase_msg_type(v_mac_address, 0, '', 'purge_notice');
        dbms_aq.enqueue(queue_name         => 'purchase_msg_queue',
                        enqueue_options    => v_enqueue_options,
                        message_properties => v_message_properties,
                        payload            => v_payload,
  ...
  begin
    ...
    Begin
      ...
      If v_acc_status_flg not in ('O') Then
        exception_msg := '#帳期未開啟#';
        Raise app_exception;
      End If;
    Exception
  ...
  Function CLIENT_REGIETER_COUPOR(client_id      varchar2,
    ...
    v_ref_purchase_pk_no number(16);
  ...
  begin
    ...
    begin
      Select program_id,
             ...
             a.serial_id,
             a.ref_device_id,
             a.ref_purchase_pk_no,
             a.CURRENT_REGISTED,
             a.CLIENT_REGISTED_LIMIT
        into v_program_id,
             v_pk_no,
             v_expire_date,
             v_src_no,
       ...
       where a.coupon_id = coupon_no
         ...
         for update;
      if v_current >=  v_max_regieted then
  ...
  /*  declare
    ...
    begin
      select 'x'
        into v_dup
        from bsm_coupon_mas a
       where a.ref_purchase_pk_no = v_ref_purchase_pk_no
         and a.serial_id = v_client_id
         and a.status_flg = 'Z'
         and rownum <= 1;
      raise dup_client_registed; 
    exception
      when no_data_found then
        null;
    end; */
  ...
  /*  declare
    ...
    begin
      ...
      if v_client_prog_limit > 0 then
        if p_device_id is not null then
              select nvl(count(*), 0)
            into v_cnt
            from bsm_purchase_mas b,bsm_coupon_mas a
    ...
    declare
      v_msg varchar2(1024);
    begin
      v_msg := bsm_purchase_post.COUPON_COMPLETE_R(0,
                                                   v_pk_no,
                                                   refresh_client);
    end;
  ...
  Function COUPON_COMPLETE_R(p_User_No      Number,
    ...
    v_start_service_date      date;
    v_REF_DETAIL_PK_NO        number(16);
    v_ref_purchase_item_pk_no number(16);
    v_coupon_id               varchar2(32);
  begin
    ...
    begin
      Select mas_code,
             ...
             a.REF_MAIN_DETAILS_PK_NO,
             a.ref_purchase_item_pk_no,
             a.coupon_id
        Into v_mas_code,
             v_mas_no,
             v_mas_date,
             ...
             v_stock_broker,
             v_stop_service_date,
             v_start_service_date,
             v_REF_DETAIL_PK_NO,
             v_ref_purchase_item_pk_no,
             v_coupon_id
        ...
        For i_Items In c1 Loop
          ...
          Exception
            When No_Data_Found Then
              select 'E'
                into v_type
                from bsm_package_options d
               where d.stk_package_id = v_id
                 and rownum <= 1;
          End;
    ...
    if v_stop_service_date is not null then
      ...
      begin
        select pk_no
         ...
         where src_pk_no = v_purchase_pk_no;
        update bsm_client_details a
           set a.end_date         = v_stop_service_date,
               a.start_date       = v_start_service_date,
               a.REF_DETAIL_PK_NO = V_REF_DETAIL_PK_NO
         where pk_no = v_dtl_pk_no;
      
        if v_REF_DETAIL_PK_NO is not null then
          update bsm_client_details a
             set a.ref_detail_pk_no = v_dtl_pk_no
           where pk_no = V_REF_DETAIL_PK_NO;
        end if;
      exception
        when no_data_found then
          null;
    ...
    if v_ref_purchase_item_pk_no is not null then
      update bsm_purchase_item a
         set a.package_dtls = replace(a.package_dtls,
                                      '_CLIENT' || v_mas_no || '_',
                                      v_client_Id)
       where a.pk_no = v_ref_purchase_item_pk_no;
    
    end if;
  
    update bsm_coupon_mas a 
    set status_flg = case when a.CURRENT_REGISTED+1 >=CLIENT_REGISTED_LIMIT then 'Z' else 'P' end
    ,CURRENT_REGISTED=nvl(CURRENT_REGISTED,0)+1 where pk_no = p_pk_no;
  ...
  begin
    ...
    begin
      select status_flg
       ...
       where a.cup_program_id = v_program_id
         and rownum <= 1;
      if v_prog_status <> 'P' then
        exception_msg := '#方案狀態錯誤' || v_program_id || '#';
        raise app_exception;
      end if;
    end;
  ...
  begin
    ...
    v_end_date := trunc(v_start_date + duration_day - (1 / (24 * 60 * 60))) +
                  (1 - (1 / (24 * 60 * 60)));
    return v_end_date;
    ...
    for c1rec in c1 loop
      ...
      if not v_GMD2_flg then
          ...
          if c1rec.pk_no is not null then
            ...
            begin
              select 'x'
               ...
               where a.src_item_pk_no = c1rec.pk_no
                 and status_flg = 'P';
        ...
        if v_create_sub then
          ...
          dbms_output.put_line(c1rec.package_id || v_package_service_type);
          begin
            Select acl_duration,
                   acl_quota,
                   package_cat1,
  ...
  begin
    update bsm_purchase_mas a
       set a.show_flg = 'N', a.recurrent = 'O', a.status_flg = 'C'
 ...
 procedure refresh_bsm_client(v_client_id varchar2) is
  begin
    ...
    begin
      ...
      v_param_length := length(v_param);
      Req            := Utl_Http.Begin_Request(link_set.link_set.p_bsm_json_url||'/BSM_Purchase_Info.ashx',
                                               'POST',
                                               'HTTP/1.1');
