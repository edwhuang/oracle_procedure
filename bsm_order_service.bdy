create or replace package body iptv.BSM_ORDER_SERVICE is
  function create_order(p_order varchar2) return varchar2 is
    p_pk_no           number(16);
    v_order           varchar2(3096);
  ...
  begin
   ...
   /* if r_order.promotion_code = 'PRO5' then
    ...
    if r_order.pay_type = 'CREDIT' then
                v_create_response := BSM_LIPAY_GATEWAY.Accepayment(r_order.pk_no,
                                                         r_order.price,
                                                         r_order.card_type,
                                                         v_src_card_no,
                                                         r_order.card_expiry,
  ...
  /*  v_create_response := bsm_payment_gateway.AccePayment(r_order.pk_no,
    ...
    commit;
      ...
      v_create_response := 'PRC=0';
    end if;
    
    if instr(v_create_response, 'PRC=0') > 0 then
            update bsm_order_mas a
         set a.status_flg = 'P'
       where pk_no = r_order.pk_no;
      commit;
      if v_create_response <>'PRC=0' 
       then
