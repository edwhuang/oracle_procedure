CREATE OR REPLACE PACKAGE BODY IPTV."BSM_CDI_SERVICE" is
...
';
  ...
  begin
    ...
    */
    ...
    function append_cdi_new_sub(p_serial_id varchar2,package_id varchar2,start_date date,end_date date,tr_id varchar2) return varchar2
  is
    req            utl_http.req;
    resp           utl_http.resp;
    rw             varchar2(32767);
    ...
    cursor c1(p_client_id varchar2) is
      select '{"transaction_id" :"' || "transaction_id" || '",' ||
       ...
       union all
       select '{"transaction_id" :"' ||tr_id || '",' ||
             '"package_id": "' || PACKAGE_ID || '",' || '"created":  "' ||
             to_char(sysdate - (8 / 24), 'YYYY-MM-DD HH24:MI:SS') || '",' ||
             '"last_modified": "' ||
             to_char(sysdate - (8 / 24), 'YYYY-MM-DD HH24:MI:SS') || '",' ||
             '"device_id":null'|| ',' ||
             '"start_time" :"' ||
             to_char(nvl(start_date,
                         to_date('2000/01/01 00:00:00',
                                 'YYYY/MM/DD HH24:MI:SS')) - (8 / 24),
                     'YYYY-MM-DD HH24:MI:SS') || '",' || '"end_time" : "' ||
  ...
  begin
    ...
    if refresh_queue is null then
      ...
      if  p_serial_id is not null and p_serial_id <> 'null'  then
     /* rw_result := link_set.link_set.post_to_cdi(v_param);  */
   ...
   function saveclientinfo(p_serial_id   varchar2,
    ...
    cursor c1(p_client_id varchar2) is
          select '{"transaction_id" :"' || nvl("transaction_id",b.pk_no) || '",' ||
             '"package_id": "' || nvl("package_id",b.package_id) || '",' || '"created":  "' ||
             to_char("created" - (8 / 24), 'YYYY-MM-DD HH24:MI:SS') || '",' ||
             '"last_modified": "' ||
             to_char("last_modified" - (8 / 24), 'YYYY-MM-DD HH24:MI:SS') || '",' ||
             '"device_id":' ||
             decode("device_id", null, 'null', '"' || "device_id" || '"') || ',' ||
             '"payment_method":' ||
            decode(Pay_type, null, 'null','贈送','"gift"','兌換券','"coupon"','信用卡','"credit"', '"' || Pay_type || '"') || ',' ||
             '"start_date" :"' ||
             ...
             '"free_start_date" :' ||
            decode("free_start_time",null,'null','"'||to_char("free_start_time" - (8 / 24),'YYYY-MM-DD HH24:MI:SS')||'"') || ','  ||
             '"free_end_date" :' ||
            decode("free_end_time",null,'null','"'||to_char("free_end_time" - (8 / 24), 'YYYY-MM-DD HH24:MI:SS')||'"') || '}' json_str
        from acl.subscription a,bsm_client_details b,bsm_purchase_mas c
       where  b.pk_no = a."transaction_id"  (+)
       and c.mas_no (+)=b.src_no
        and b.serial_id =p_client_id
        and b.status_flg='P';
  
  begin
    if refresh_queue is null then
      select owner_phone into v_phone_no from bsm_client_mas c where c.serial_id=p_serial_id;
    ...
    "id":"67867878",
    "method": "httpRequest", 
    "params": { "url" : "https://setaccinfo.svc.litv.tv/lambda/SaveAccountInfo", "postData" : {
  "id": "TEST",
  "jsonrpc": "2.0",
  "method": "setAccountInfo",
  "params": {
        "client_id": "'||p_SERIAL_ID||'",
        "phone_no":"'||v_phone_no||'",
        "Subscriptions":'||json_str||'}}}}';
      v_param  := replace(v_param,
                          '_RPCID_',
  ...
  /*   begin
     ...
     exception
      ...
      begin
        v_param_length := length(v_param);
        Req            := Utl_Http.Begin_Request(link_set.link_set.p_bsm_json_url||'/bsm_purchase_service.ashx',
                                                 'POST',
                                                 'HTTP/1.1');
 ...
 /*        begin
    ...
    */
      ...
      declare
        ...
        v_payload            purchase_msg_type;
      begin
        v_payload := purchase_msg_type(p_serial_id, 0, '', 'saveclientinfo');
        dbms_aq.enqueue(queue_name         => refresh_queue,
                        enqueue_options    => v_enqueue_options,
                        message_properties => v_message_properties,
                        payload            => v_payload,
