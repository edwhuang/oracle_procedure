CREATE OR REPLACE PROCEDURE IPTV."TPM_OUT" is
   cursor c_mas is 
     select distinct whs_id, tr_id, cust_id, tr_date, to_date(return_date,'yyyymmdd') return_date, remark  
       from leased_temp t 
      where t.code = 'D'   
        and whs_id = 'HZ-K' --in ('TP-M','TP-A')
        order by whs_id; 

   cursor c_items(i_tr_id varchar2) is  
     select stock_id, unit, (qty-nvl(return_qty,0)) qty, 0 return_qty 
       from leased_temp t 
      where t.code = 'D' 
        --and whs_id = 'TP-N'
        and tr_id = i_tr_id; 
...
dbms_output.put_line('start');
   open c_mas; 
   fetch c_mas into v_whs_id, v_tr_id, v_cust_id, v_trx_date, v_return_date, v_remark; 
   loop 
      ...
      select seq_inv_no.nextval
        into v_mas_no 
        from dual;      
      v_pl_id := sysapp_util.get_mas_no(1,1,to_date('20081201','yyyymmdd'),'TRXINVS',v_mas_no);
dbms_output.put_line(v_tr_id||'- '||v_pl_id); 
      ...
      fetch c_items into v_stock_id,v_unit,v_qty,v_return_qty; 
      loop 
