create or replace procedure iptv.modify_service_1111_p6_b(pi_client_id varchar2 default null) is
begin
  DECLARE
    cursor c0 is
           with t1 as
       (Select a.pk_no purchase_pk_no,
               a.serial_id,
               a.mas_no,
               a.pay_type,
               c.start_date,
         ...
         /*  and pay_type in ('信用卡',
                            ...
                            'GOOGLEPLAY',
                            'IOS',
                            'SONET')  */ -- google ,ios 的豪華要判斷但不搬
           and d.package_id = c.package_id
           and b.package_id = d.package_id
           and c.src_pk_no = a.pk_no
           and c.status_flg = 'P'
           AND a.SERIAL_ID =pi_client_id
           and not exists (select 'x'
                  from bsm_issue_item x
                 where x.modify_check like 'a.pk_no' || '%'
                   and rownum <= 1) )
      select  purchase_pk_no,serial_id,mas_no,dup_pur_date,package_cat_id1,dup_start_date,process_sts
        from (SELECT t6.purchase_pk_no,
                     t6.serial_id,
                     t6.mas_no,
               ...
               where t2.status_flg = 'P'
                 ...
                 and t1.purchase_pk_no!=t2.src_pk_no
                and (t1.package_cat_id1 = 'VOD_CHANNEL_DELUX'-- or
                 or     t1.package_cat_id1 = t5.package_cat_id1
 )
                 and (t2.package_id not in ('FREE00',
                                            'FREE98',
                                            'FREE99',
                                            'FREE01',
                                            'FREE95',
               ...
               AND t6.package_cat_id1=t5.PACKAGE_CAT_ID1
                and t6.end_date > = t2.end_date
                 and (t2.serial_id =   pi_client_id)
                 and (t3.pay_type not in ('兌換券') or
                     (t3.pay_type = '贈送' and t3.promo_code like 'promo%'))
                 and t4.serial_id = t3.serial_id
                 and not exists
               (select 'x'
                        from bsm_issue_item x
                       where x.modify_check = t1.purchase_pk_no ||
                             t1.end_date || ' ' || t3.mas_no))
       where process_sts = 'A'
         and (xbuy_date >= sysdate - 30 or dup_pur_date >= sysdate - 30)
         group by  purchase_pk_no,serial_id,mas_no,dup_pur_date,package_cat_id1,dup_start_date,process_sts;
    CURSOR C1(p_purchase_pk_no number) is
      with cte as
       (select *
          from (Select t4.owner_phone,
                       t1.purchase_pk_no,
                       t1.serial_id,
                       t1.mas_no,
                       t1.pay_type,
                       ...
                       case
                         when t3.pay_type not in ('信用卡',
                                                  ...
                                                  'iab_tv') then
                          null
                         when trunc(t2.start_date) >= trunc(t1.start_date) and
                              sysdate > trunc(t1.start_date) then
                       ...
                       case
                         when t3.pay_type not in ('信用卡',
                                                  ...
                                                  'iab_tv') then
                          null
                         when t2.start_date >= t1.start_date and
                              sysdate > t1.start_date then
                       ...
                       case
                         ...
                         when trunc(t2.start_date) < trunc(t1.start_date) and
                              trunc(t1.end_date) < trunc(t2.end_date) then
                 ...
                 where t2.status_flg = 'P'
                   and t2.src_pk_no = t3.pk_no
                   and t5.package_id = t2.package_id
                   and t2.src_pk_no in p_purchase_pk_no

                   and (t1.package_cat_id1 = 'VOD_CHANNEL_DELUX' or
                       t1.package_cat_id1 = t5.package_cat_id1)
                   and (t2.package_id not in ('FREE00',
                                              'FREE98',
                                              'FREE99',
                                              'FREE01',
                   ...
                   and t2.end_date >= t1.start_date
                   and t2.start_date < t1.end_date
                   and t2.src_pk_no <> t1.purchase_pk_no
                   and t2.serial_id = t1.serial_id
                   and (t2.serial_id = pi_client_id)
                   and (t3.pay_type not in ('兌換券') or
                       (t3.pay_type = '贈送' and t3.promo_code like 'promo%'))
                   and t3.serial_id not in
                       ('2A00DBA59350E400',
                        '2A00DEDF6A736900',
                        '2A00EB267F728DC2',
                        '2A00028624FC360E')
      ...
      select *
        from cte
       where (cte.serial_id, dup_start_date, package_cat_id1) in
             (select serial_id, min(dup_start_date), package_cat_id1
                from cte
               group by serial_id, package_cat_id1)
       order by start_date;
    /* with cte as (
     select * from (
     Select t4.owner_phone,
            ...
            from (Select a.pk_no purchase_pk_no,
                   ...
                   where b.mas_pk_no = a.pk_no
                     ...
                     and d.package_id=c.package_id
                     and c.end_date >= sysdate
                     and pay_type in ('信用卡','手動刷卡', '信用卡二次扣款','ATM','REMIT','贈送','GOOGLEPLAY','IOS','SONET')
                   --  and c.package_id like 'XD%'
                     and c.src_pk_no = a.pk_no
                     and c.status_flg='P'
           ...
           where t2.status_flg = 'P'
             ...
             and (t1.package_cat_id1='VOD_CHANNEL_DELUX' or t1.package_cat_id1=t5.package_cat_id1)
  ...
  begin
    ...
    for j in c0 loop
      for i in c1(j.purchase_pk_no) loop
        v_purchase_pk_no:= i.purchase_pk_no;
        begin
        ...
        if i.process_sts = 'A' then
          ...
          declare
            ...
            v_client_issue_cnt number(16);

            v_item_pk_no number(16);
            cursor c1(p_client_id       varchar2,
                      p_end_date        date,
                      p_package_cat_id1 varchar,
                      e_days            number) is
              select case
                       when a.end_date > p_end_date then
                        '整體期限延後1年'
                       else
                        null
                     end method,
                     ...
                     d.mas_no,
                     c.package_cat_id1
                from bsm_purchase_mas   d,
                     bsm_client_details a,
                     bsm_package_mas    c
                 ...
                 and pay_type in ('信用卡',
                              ...
                              --    'GOOGLEPLAY',
                                --  'IOS',
                                  'thenews',
                                  'iab_tv')
                  and d.pk_No not in v_purchase_pk_no
                 and a.status_flg = 'P'
                 and d.status_flg = 'Z'
          ...
          begin
            select count(*)
              into v_client_issue_cnt
              from bsm_issue_mas c
             where trunc(c.mas_date) = trunc(sysdate)
               and c.client_id = pi_client_id;
            if v_client_issue_cnt < 30 then
              ...
              for j in c1(i.serial_id,
                ...
                if nvl(j.method, i.method) = '整體期限延後1年' then
                  ...
                  begin
                    select 'x'
                      into v_char
                      from bsm_issue_item b, bsm_issue_mas a
                     where dtl_pk_no = i.dtl_pk_no
                       and a.pK_no = b.mas_pk_no
                       and b.change_type = 'N'
                       and a.mas_date >= sysdate - (3 / (24 * 60));
              ...
              v_msg := bsm_issue_post.bsm_issue_complete(0,
                                                         v_issue_pk_no
                                                         ,'N');
              ...
              update MODIFY_SERVICE_1111 t
               ...
               where t.purchase_pk_no = i.purchase_pk_no
                 and t.dup_pk_no = i.dup_pk_no;
                    v_msg := bsm_cdi_service.refresh_client(pi_client_id ,'purchase_msgb_queue');
    bsm_client_service.refresh_bsm_client(pi_client_id ,'purchase_msgb_queue');
  ...
  declare
    cursor c1(p_client_id varchar2) is
      select t1.serial_id, mas_no, t1.pk_no
        from bsm_purchase_mas t1,
