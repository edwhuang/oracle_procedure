CREATE OR REPLACE FUNCTION IPTV."GET_DEVICE_TOKEN"
...
BEGIN
  if client_id is not null then
    if upper(substr(client_id,1,2)) = 'AA' then
       content := content_client;
    else
       content := content_device;
    end if;
...
req := utl_http.begin_request(link_set.link_set.cdi_manager_url,'POST','HTTP/1.1');
 ...
 --   values(sysdate,client_id,content,pagelob);
  --  commit;
  return json_ext.get_string(jsonobj,'result.token');
  --
