CREATE OR REPLACE PACKAGE BODY IPTV."BSM_IOS_GATEWAY_DEV" is

  -- office
  -- PAYMENT_GATEWAY_URL varchar2(256):='http://172.21.200.241:5600/NewebPayment/CCAccept';

  -- colo
  ios_storeURL varchar2(256) := link_set.link_set.p_bsm_json_url||'/BSM_JSON_SERVICE/BSM_Purchase_Service.ashx';
--  ios_storeURL varchar2(256) := 'https://buy.itunes.apple.com/verifyReceipt';
  ...
  begin
    ...
    rw_result := bsm_ios_gateway_dev.get_Receipt_Data(p_order_no,
                             p_receipt,
                             p_password,
                             p_ios_product_code);
  ...
  is
    ...
    status_flg varchar2(32);
  begin
    rw_result := bsm_ios_gateway_dev.get_Receipt_Data(p_order_no,'','',p_ios_product_code);
    jsonobj:=json(rw_result);
