create or replace procedure iptv.modify_service_1111_p7(pi_client_id varchar2 default null) is
begin
  DECLARE
    cursor c0 is
      with t1 as
    ...
    /*       and not exists (select 'x'
      ...
      select purchase_pk_no,
       ...
       group by purchase_pk_no,
                ...
                process_sts,
                t6_star
       order by t6_star, dup_start_date;
          ...
          from (Select t4.owner_phone,
                       ...
                       case
                         ...
                         when trunc(t2.start_date) >= trunc(t1.start_date) and
                          ...
                          trunc(t2.end_date + (trunc(t1.end_date) - trunc(sysdate))+1)
                         when trunc(t2.start_date) >= trunc(t1.start_date) and
                              sysdate <= t1.start_date then
                          trunc(t2.end_date + (t1.end_date - t1.start_date)+1)
                         when trunc(t2.start_date) < trunc(t1.start_date) then
  ...
  begin
    ...
    for j in c0 LOOP
      ...
      if v_client_issue_cnt < 30 and v_issue_pk_no = 0 then
                  ...
                  begin
                    select 'x'
                     ...
                     where dtl_pk_no = i.dtl_pk_no
                       ...
                       and b.change_type = 'N'
                       and a.mas_date >= sysdate - (1 / (24 * 60 * 60));
        ...
        IF v_issue_pk_no > 0 and item_cnt >0 THEN
          v_msg := bsm_issue_post.bsm_issue_post(0, v_issue_pk_no);
          v_msg := bsm_issue_post.bsm_issue_complete(0, v_issue_pk_no, 'N');
        
          v_msg := bsm_cdi_service.refresh_client(pi_client_id,
                                                  'purchase_msgb_queue');
          bsm_client_service.refresh_bsm_client(pi_client_id,
                                                'purchase_msgb_queue');
        END IF;    
    end loop;
  ...
  declare
    cursor c1(p_client_id varchar2) is
      select t1.serial_id, mas_no, t1.pk_no,t1.pay_type
        from bsm_purchase_mas t1,
             (Select b.serial_id, min(b.pk_no) min_pk_no, max(b.mas_no)
               ...
               --  and a.recurrent_type in ('LiPayN', 'LiPay','IOS','IAB')
  ...
  begin
    for i in c1(pi_client_id) loop
      ...
      exception
         when no_data_found then */
           if i.pay_type in ('信用卡','CREDIT') then
