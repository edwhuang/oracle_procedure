create or replace procedure iptv.crt_purchase(p_paytype    varchar2,
...
begin
  insert into tmp_ios_check(log_stg) values (p_paytype||','||   
                                         p_client_id||','||  
                                         p_device_id||','||  
                                         p_package_id||','|| 
                                         p_src_no||','||     
  ...
  begin
    select a.pk_no, a.serial_id, a.purchase_date, b.package_id
      into purchase_pk_no, org_client_id, org_purchase_date, org_package_id
      from bsm_purchase_mas a, bsm_purchase_item b,bsm_recurrent_mas c
     where a.status_flg = 'Z'
       and a.src_no = p_src_no
       and b.package_id = p_package_id
       and pay_type = 'IOS'
  ...
  exception
    ...
    when no_data_found then
      begin
        select pk_no, serial_id, purchase_date, package_id into
                purchase_pk_no,
               org_client_id,
               org_purchase_date,
               org_package_id
        ...
        if org_client_id = p_client_id and org_package_id <> p_package_id then
        
          update bsm_recurrent_mas t
             set t.status_flg = 'N', t.remark = 'IOS 更換方案 ' || p_pk_no
           where t.src_pk_no = purchase_pk_no;
          update bsm_purchase_mas t
             set t.src_no = t.src_no
           where pk_no = purchase_pk_no;
          commit;
          bsm_client_service.Set_subscription(null, org_client_id);
          v_method := 'create';
        else
          v_method := 'create';
        end if;
