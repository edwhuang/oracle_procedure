CREATE OR REPLACE FUNCTION IPTV."GET_CLIENT_VAL"(client_id      varchar2,
                                            p_name         varchar2,
                                            p_default_val  clob,
                                            p_software_ver varchar2 default null,
                                            device_id      varchar2 default null)
  return clob is
  v_val            clob;
  ...
  begin
    ...
    if (p_software_ver is not null) and
      ...
      begin
        ...
        if substr(v_software_group, 1, 7) <> substr(p_software_ver, 1, 7) then
          v_software_group := substr(p_software_ver, 1, 7);
        end if;
      end;
