CREATE OR REPLACE PROCEDURE IPTV."ACL_MASTER_TRANSFER" is
...
begin
  ...
  begin
   -- update /*+ NOLOGGING */ acl.object set "deleted" = 1;
    for c1rec in c1 loop
     ...
     /*   update + NOLOGGING  acl.object
           set "object_id"     = c1rec.object_id,
               "type"          = c1rec.type,
               "deleted"       = c1rec.deleted,
               "last_modified" = c1rec.last_modified
         where "object_id" = c1rec.object_id; */
      exception
        when no_data_found then
          insert /*+ Append */ into acl.object
  ...
  begin
   -- update /*+ NOLOGGING */ acl.package set "deleted" = 1;
    for c1rec in c1 loop
      begin
        select 'x'
          into v_char
    ...
    /*    update + NOLOGGING  acl.package
           set "package_id"      = c1rec."package_id",
               "duration"        = c1rec."duration"*100,
               "quota"           = c1rec."quota",
               "level"           = c1rec."level",
               "last_modified"   = c1rec."last_modified",
               "deleted"         = null,
               "throttled"       = c1rec."throttled",
               "period"          = c1rec."period",
               "period_duration" = c1rec."period_duration"*100,
               "token"           = c1rec."acl_token",
               "ad"              = c1rec."ad"
         where "package_id" = c1rec."package_id"; */
      exception
        when no_data_found then
          insert /*+ Append */ into acl.package
            ("package_id",
             "duration",
             "quota",
             "level",
    ...
    cursor c1 is
     ...
     union all          
       ...
       to_date('20111231', 'YYYYMMDD') "expiration",
        ...
        from mid_cms_vod_channel a,bsm_package_mas b 
        where a.acl_type=0
        and b.package_cat_id1 like 'CHANNEL'
  union all
       Select package_id  "parent_id",cdn_code "child_id",
       to_date('20111231', 'YYYYMMDD') "expiration",
             null "deleted",
  ...
  union all
       ...
       to_date('20111231', 'YYYYMMDD') "expiration",
             ...
             sysdate "last_modified"
        from mid_cms_vod_channel a,bsm_package_mas b 
        where (a.acl_type=0 or a.acl_type=4)
          and (b.package_cat_id1 = 'VCHANNEL_CABLE')   
       union all  
      Select acl_id  "parent_id",d.content_id "child_id",
       to_date('20111231', 'YYYYMMDD') "expiration",
             null "deleted",
             sysdate "last_modified"
        from MID_CNS_CONTENT a,bsm_package_mas b ,iptv.ccc_program_asset d
        where a.series_id=d.serial_no and b.package_cat_id1='VOD_CABLE'
        and d.active_flg='Y' 
        group by  acl_id ,d.content_id                 
      union all
       Select package_id  "parent_id",cdn_code "child_id",
       to_date('20111231', 'YYYYMMDD') "expiration",
             null "deleted",
     ...
     Select package_id  "parent_id",cdn_code "child_id",
       to_date('20111231', 'YYYYMMDD') "expiration",
             null "deleted",
             sysdate "last_modified"
        from mid_smallpak_channel a,bsm_package_mas b 
        where ((a.pack_id =1)
         and ( b.package_cat_id1 like 'BLESSEDLIFE%'))
         or ((a.pack_id=2)
         and ( b.package_cat_id1 like 'TOPMSO%'))         
    union all  
             Select package_id  "parent_id",cdn_code "child_id",
       to_date('20111231', 'YYYYMMDD') "expiration",
             null "deleted",
   ...
   and /*((b.package_cat_id1 like 'S_VOD_CHANNEL%' or
       ...
       (sysdate > to_date('20170620100000', 'YYYYMMDDHH24MISS') and
       b.package_cat_id1 in
       ('CHANNEL', 'VOD_CHANNEL', 'VOD_CHANNEL_RB1', 'VOD_CHANNEL_RB2', 'VOD_CHANNEL_RB3', 'VOD_CHANNEL_RB4') and
       ( b.system_type = 'BUY' or b.package_id ='CHG003') and b.status_flg = 'P' and b.package_id not in ('WDS001','WD0006','WD0007'))) */
       
       ((b.package_cat_id1 like 'S_VOD_CHANNEL%' or
       ...
       ( b.system_type = 'BUY' or b.package_id ='CHG003') and b.status_flg = 'P' and  b.package_id not in ('WDS001','WD0006','WD000S'))) 
         ...
         where not exists(select 'x' from acl.relationship t1 where t1."parent_id"=t2."parent_id" and t1."child_id"=t2."child_id" and t2."deleted" is null)
...
Select b.acl_id,content_id,to_date('2011/12/31','YYYY/MM/DD'),null,sysdate from iptv.ccc_program_asset a ,ccc_free_serialno b
 where a.serial_no=b.serial_no and a.episode_no=b.episode_no and b.start_date <= trunc(sysdate) and trunc(b.end_date)> trunc(sysdate)
   and a.active_flg = 'Y'
   and b.method='add'
    and not exists(select 'x' from acl.relationship t1 where t1."parent_id"=b.acl_id and t1."child_id"=content_id)
group by content_id,b.acl_id;

declare
  cursor c1 is
select * from (select t1.rowid rid,t1.*,cte.content_id from acl.relationship t1, (
select distinct a.content_id from iptv.ccc_program_asset a ,ccc_free_serialno b
 where a.serial_no=b.serial_no  and  b.start_date <= trunc(sysdate) and  trunc(b.end_date)> trunc(sysdate)  and ( b.episode_no is null or b.episode_no=a.episode_no)
   and a.active_flg = 'Y' ) cte where "parent_id" like 'FREE%' and "deleted"=1 
and t1."child_id" =cte.content_id (+)) cte2 where cte2.content_id is null;
begin
  for i in c1 loop
      update acl.relationship t3
      set t3."deleted"=null
      where t3.rowid=i.rid;
...
Select a.content_id
  from ccc_program_asset a where a.serial_no in (select distinct serial_no from ccc_free_serialno a where a.start_date <= trunc(sysdate) and  trunc(a.end_date)> trunc(sysdate) and a.method='erase' and a.episode_no is null)); 
...
/* 
  update acl.relationship b 
    set b."deleted"=1
  where b."parent_id" like 'FREE%' and b."child_id"='VOD00085003';
 */
...
commit;
 ...
 -- null;
  ...
  values
    (sysdate, 0, 'CONTENT_ITEM_POST', 'END ACL JSON call :' || v_msg);
  commit;
