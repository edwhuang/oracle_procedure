create or replace procedure iptv.purchase_callback_procedure(purchase_msg purchase_msg_type) AS

begin
    if instr(purchase_msg.process_type ,'promo_') > 0 then
    declare
      v_msg varchar2(1024);
    BEGIN
     v_msg:=promo_activate_30day(purchase_msg.process_type,purchase_msg.client_id,null,purchase_msg.purchase_no);
    END;
    null;

  elsif purchase_msg.process_type = 'promo' then
   ...
   elsif purchase_msg.process_type = 'saveclientinfo' then
    declare
      v_msg clob;
    begin
      ...
      if purchase_msg.client_id = 'F6AEF1815EC63D2E' then
       ...
       -- v_msg := bsm_cdi_service.refresh_client(purchase_msg.client_id);
        v_msg := bsm_cdi_service.saveclientinfo(purchase_msg.client_id);
      end if;
    end;
      ...
      cursor c1 is
         Select b.src_pk_no,
               ...
               trunc(bsm_recurrent_util.get_service_end_date(c.package_cat_id1,
                                                             a.client_id)) <= trunc(sysdate) then
                sysdate+1
                else
                 trunc(bsm_recurrent_util.get_service_end_date(c.package_cat_id1,
                                                            a.client_id)) 
    ...
    declare
     ...
     begin  
        v_msg := bsm_cdi_service.purgeNotice(purchase_msg.client_id);
      end;
  end if;
